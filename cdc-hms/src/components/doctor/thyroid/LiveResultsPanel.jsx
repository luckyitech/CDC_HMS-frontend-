...
export default function LiveResultsPanel({ report, nodules }) {
  ...
  return (
    <div className="space-y-3">
      <div className="text-xs font-semibold text-slate-400 uppercase tracking-wide">Live results</div>

      <div className="bg-white rounded-xl border border-slate-200 p-3">
        <div className="text-[11px] text-slate-400 uppercase tracking-wide mb-1.5">Volumes (mL)</div>
      ...
      <div className="bg-white rounded-xl border border-slate-200 p-3">
        <div className="text-[11px] text-slate-400 uppercase tracking-wide mb-1.5">Nodules</div>
        {!nodules.length ? (
          <div className="text-xs text-slate-400">{report.noNodules ? 'No nodules identified' : 'None entered yet'}</div>
        ) : (
          <div className="space-y-1.5">
            {nodules.map((n) => {
              const t = engine.computeTirads(n);
              const cat = t.insufficient ? 'TR—' : t.category;
              const col = t.insufficient ? 'bg-slate-400 text-white' : TR_COLOR[t.category];
              ...
              return (
                <div key={n.id} className="flex items-center justify-between text-xs">
                  <span className="text-slate-600">Nodule {n.noduleNumber}</span>
                  <span className="flex gap-1">
                    <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded ${col}`}>{cat}</span>
                    {n.btaCategory && <span className="text-[10px] font-bold px-1.5 py-0.5 rounded bg-slate-700 text-white">{n.btaCategory}</span>}
      ...
      <div className="bg-white rounded-xl border border-slate-200 p-3">
        <div className="text-[11px] text-slate-400 uppercase tracking-wide mb-1.5">Before signing</div>
...
function Stat({ label, value, accent }) {
  return (
    <div>
      ...
      <div className={`font-semibold ${accent ? 'text-teal-700' : ''}`}>{value != null ? value : '—'}</div>
    </div>
  );
}
